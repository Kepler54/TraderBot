...
class DataAnalysis(DataBase):
    ...
    def buy_sell_verify(self) -> str:
        ...
        :return: str
        """
        if float(self.percentage(self.coin_trade_info(self.sell)[-1], self.sell)[0]) >= self.get_notification()[0]:
            return f", нужно покупать [{self.get_coin_name(self.coin_second)}]"
        elif float(self.percentage(self.coin_trade_info(self.buy)[-1], self.buy)[0]) <= -self.get_notification()[0]:
